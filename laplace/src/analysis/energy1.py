...
def compute_energies(topo_file, traj_file, filename):
    ...
    with open(filename, 'w') as f:
        for step in sorted(traj.keys()):
            ...
            print("{}: {}".format(step, total_kinetic_energy), file=f)

        if perform_binning:
            print("BINS: {}".format(bins), file=f)
            print("SPIKED: {}".format(spike_at), file=f)

def main():
    compute_energies(sys.argv[1], sys.argv[2], sys.argv[3])
